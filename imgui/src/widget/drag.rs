    ...
    M: AsRef<str>,
{
    ...
    pub fn build(self, ui: &Ui, min: &mut f32, max: &mut f32) -> bool {
        ...
        unsafe {
            ...
            let label_start = buffer.push_len(self.label);
            let display_format = self.display_format.as_ref().map(|v| buffer.push_len(v));
            let max_display_format = self.max_display_format.as_ref().map(|v| buffer.push_len(v));
